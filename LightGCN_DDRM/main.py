...
from model import LightGCN, DNN, DDRM
...
class Trainer(object):
    def __init__(self, data_config, n_users, n_items, graph, warm_adj, confidence_score, vv_count):
        ...
        self.out_dims = eval(args.dims) + [args.recdim]
        self.in_dims = self.out_dims[::-1]

        self.user_reverse_model = DNN(self.in_dims, self.out_dims, args.embed_size, time_type="cat", norm=args.norm).cuda()
        self.item_reverse_model = DNN(self.in_dims, self.out_dims, args.embed_size, time_type="cat", norm=args.norm).cuda()
        ...
        self.diffusion = gd.GaussianDiffusion(world.config, mean_type, args.noise_schedule, \
        args.noise_scale, args.noise_min, args.noise_max, args.steps, world.device).cuda()

        self.model = DDRM(self.n_users, self.n_items, self.emb_dim, n_layers = 3, graph = self.graph)
        self.model = self.model.cuda()

        self.diff_lr = args.diff_lr
        self.alpha = args.alpha
        self.joint1 = args.joint1
        self.joint2 = args.joint2

        self.optimizer_D = optim.Adam([{'params':self.model.parameters()},], lr=self.lr)
        self.opt_user_dnn = optim.Adam(self.user_reverse_model.parameters(), lr=self.diff_lr)
        self.opt_item_dnn = optim.Adam(self.item_reverse_model.parameters(), lr=self.diff_lr) 
    ...
    def test(self, users_to_test, users_to_test_cold, users_to_test_warm, is_val):
        self.model.eval()
        with torch.no_grad():
            users_emb, items_emb = self.model.computer()
            # users_emb, items_emb, _ = self.model.computer(users, pos_items, self.user_reverse_model, self.item_reverse_model, self.diffusion)
    ...
    def train(self):
        ...
        for epoch in range(args.epoch):
            ...
            for idx in tqdm(range(n_batch)):
                ...
                batch_bpr_loss, batch_reg_loss, batch_re_loss, pos_score, all_users, all_items = self.model.bpr_loss(users, pos_items, neg_items, self.user_reverse_model, self.item_reverse_model, self.diffusion)
                # print ('loss size: ', type(batch_bpr_loss), type(batch_reg_loss), type(batch_re_loss))
                # print ('loss size: ', batch_bpr_loss.size(), batch_reg_loss.size(), batch_re_loss.size())
                ...
                weight = torch.pow(pos_score_, world.config['beta'])

                batch_loss = (1 - self.alpha * self.joint1) * batch_loss + self.alpha * self.joint1 * batch_re_loss
                ...
                batch_denoise_loss = torch.sum(-re_labels * torch.log2(pred) - (1 - re_labels) * torch.log2(1 - pred))
                batch_loss = batch_loss + self.joint2 * batch_denoise_loss
                ...
                re_loss += float(batch_re_loss.mean())
                bpr_loss += float(batch_bpr_loss.mean())
                # ce_loss += float(batch_ce_loss)
                reg_loss += float(batch_reg_loss.mean())

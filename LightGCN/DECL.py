...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(args.epoch):
            t1 = time()
            loss, bpr_loss, reg_loss, ce_loss, edl_loss = 0., 0., 0., 0., 0.
            ...
            for idx in tqdm(range(n_batch)):
                ...
                batch_loss =  batch_bpr_loss + 0.0001 * batch_edl_loss + batch_reg_loss
            ...
            if (epoch + 1) % args.verbose != 0:
                perf_str = 'Epoch %d [%.1fs]: train==[%.5f=%.5f + %.5f + %.5f]' % (
                    epoch, time() - t1, loss, bpr_loss, edl_loss, reg_loss)

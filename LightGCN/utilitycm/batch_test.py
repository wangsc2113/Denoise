import utilitycm.metrics as metrics
from utilitycm.parser import parse_args
from utilitycm.load_data import Data
...
def cold_test_one_user(x):
    ...
    if is_val:
        ...
        user_pos_test = data_generator.cold_test_set[u]
...
def warm_test_one_user(x):
    ...
    if is_val:
        ...
        user_pos_test = data_generator.warm_test_set[u]
...
def cold_test_torch(ua_embeddings, ia_embeddings, users_to_test, is_val, drop_flag=False, batch_test_flag=False):
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)), 'hit_ratio': np.zeros(len(Ks)), 'auc': 0.}
    pool = multiprocessing.Pool(cores)
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        batch_result = pool.map(cold_test_one_user, user_batch_rating_uid)
        count += len(batch_result)
...
def warm_test_torch(ua_embeddings, ia_embeddings, users_to_test, is_val, drop_flag=False, batch_test_flag=False):
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)), 'hit_ratio': np.zeros(len(Ks)), 'auc': 0.}
    pool = multiprocessing.Pool(cores)
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        batch_result = pool.map(warm_test_one_user, user_batch_rating_uid)
        count += len(batch_result)
